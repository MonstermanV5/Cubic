...
Cubic does not contain Mojang or Microsoft code or assets. The repository currently contains the architectural scaffold, native graphics bootstrap, protocol/NBT and generated-data foundations, authenticated Chat Mode, semantic world/chunk state, accepted runtime-only official block resources, movement/collision, seamless Play/Chat presentation, accepted Phase 18B/18C fidelity, accepted Phase 19 block targeting and interaction, and the Phase 20 inventory/items implementation awaiting localhost acceptance.

## Current state

With no arguments, `cubic-app` opens the Phase 2 clear-frame window. `status`, `dev-login`, and `chat` retain their accepted behavior. `world <server> --username <name>` opens the offline-development Play window that renders bounded decoded chunks using official exact-version block resources verified and cached by Cubic. It includes accepted movement/collision, rendering fidelity, special movement, the same-session `CHAT`/`PLAY` toggle, Phase 19 interaction, and Phase 20's functional inventory overlay. `E` opens the player inventory, server menus open automatically, `Escape` closes the current server menu, and `1`-`9` or the mouse wheel selects the authoritative hotbar. Inventory clicks and drag previews update locally before server reconciliation. Creative mode consumes exact generated 26.1.2 tab metadata and ordered component-bearing stacks rather than registry order or identifier heuristics. Item icons resolve component-selected item graphs, verified model/special-family geometry, GUI transforms, and directional item lighting into the deterministic runtime atlas used by every inventory screen; dynamic special layers without sufficient live stack data retain documented static limitations. Entities remain unimplemented. See `docs/INVENTORY.md`, `docs/INTERACTION.md`, `docs/WORLD_RENDERING.md`, `docs/BLOCK_RESOURCES.md`, `docs/MOVEMENT.md`, and `docs/CHAT_MODE.md`.
