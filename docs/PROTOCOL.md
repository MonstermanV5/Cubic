...
Minecraft-version-specific packet IDs and behavior must not be scattered through this layer. The single `bootstrap::v775` exception is intentionally isolated and temporary. Its Phase 13 additions decode the reviewed clientbound Login (`0x31`), Change Difficulty (`0x0a`), Game Event (`0x26`), Initialize Border (`0x2b`), Player Position (`0x48`), Respawn (`0x52`), Set Default Spawn Position (`0x61`), and Set Time (`0x71`) layouts. Phase 14 adds reviewed `forget_level_chunk` (`0x25`), `level_chunk_with_light` (`0x2d`), and `light_update` (`0x30`) codecs. Official 26.1.2 reports remain authoritative for identities/IDs; these field layouts were independently reviewed against the pinned structural source and official 26.1.2 class codecs because the Phase 12 artifact conservatively classifies them as `IdentityOnly`. Paletted storage computes exact fixed word counts and uses non-crossing entries. `cubic-network` translates wire values to packet-ID-free world events. See `CHUNKS.md`.
