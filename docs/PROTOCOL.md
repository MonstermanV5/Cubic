...
Minecraft-version-specific packet IDs and behavior must not be scattered through this layer. The single `bootstrap::v775` exception is intentionally isolated and temporary. Its Phase 13 additions decode the reviewed clientbound Login (`0x31`), Change Difficulty (`0x0a`), Game Event (`0x26`), Initialize Border (`0x2b`), Player Position (`0x48`), Respawn (`0x52`), Set Default Spawn Position (`0x61`), and Set Time (`0x71`) layouts. Official 26.1.2 packet reports remain authoritative for identities/IDs; these layouts were independently reviewed against the pinned structural source and official 26.1.2 class codecs because the Phase 12 artifact conservatively classifies them as `IdentityOnly`. `cubic-network` translates them to version-independent world events. Unknown complete Play frames remain bounded and are immediately discarded. Clientbound Play `0x18` remains bounded Custom Payload traffic.
