...
Phase 9 adds the narrowly scoped protocol-775 Encryption Request and Encryption Response fields to the same bootstrap profile. The network transport—not packet codecs—owns AES-128/CFB8 and compressed framing. Compression uses a VarInt Data Length, zlib rather than gzip, zero for an uncompressed body, and strict compressed/decompressed bounds. General generated packet schemas remain Phase 12 work. Player Session and secure-chat signing are not yet implemented.
