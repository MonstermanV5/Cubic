...
Phase 17B applies the same rule to physical block collision. Phase 20's schema-versioned 26.1.2 shape artifact is produced by a read-only development oracle from initialized vanilla block states and stores interned normalized collision and outline AABBs. Runtime selection additionally checks identifier and the complete property map, so a sparse or mismatched dataset cannot accidentally borrow a same-numbered state. Context-dependent scaffolding, powder-snow, and moving-piston states are tagged rather than falsely described as universally static. Other versions require their own generated artifact or explicit runtime rules; they never silently inherit 26.1.2 IDs.

Phase 13 world state is not versioned wire data. Each selected protocol profile translates its packet IDs, field order, relative-position flags, registry references, and discriminants into stable `cubic-world::WorldEvent` values. The current `v775` adapter is the first reference implementation only. Future generated codecs can replace that adapter without changing `WorldState`, Chat Mode, or renderer-facing semantics.

Phase 20 adds a separate schema-versioned Creative-data profile keyed by exact Minecraft version. For 26.1.2 it records the official Creative tab registry metadata, permission variants, exact ordered component-bearing stacks, feature requirements, and canonical Search output. The generic inventory UI consumes those stable identities and flags; it contains no identifier-name category heuristics. The checked-in artifact is factual generated metadata, not a copied client asset, and a future exact version receives its own generated profile.
