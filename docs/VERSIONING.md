...
Phase 17B applies the same rule to physical block collision. Generated Phase 11 data supplies exact runtime state IDs and properties, but the official block report contains no voxel-shape behavior. An explicitly selected 26.1.2 collision-rule adapter converts those facts to bounded physical shapes; an unknown exact version receives only the conservative air/full-cube baseline. Other versions must add or generate their own reviewed rule set rather than silently inheriting 26.1.2 behavior or scattering version checks through movement code.
