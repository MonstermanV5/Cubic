...
This document describes current and intended boundaries. Phases 1-19 provide the accepted terrain, movement, rendering-fidelity, special-movement, Play/Chat, and bounded block-interaction foundation; inventory/items and entities remain unimplemented.
...
- `cubic-world`: owns connection-scoped semantic `WorldState`, lifecycle/reset rules, dimensions, player world metadata, authoritative position, spawn/time/difficulty/weather/border state, bounded runtime registries, and Phase 19's version-independent DDA raycast/outline-shape semantics. It knows no packet IDs, sockets, UI, renderer, or protocol version.
