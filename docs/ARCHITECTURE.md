...
Phase 13 follows `exact-version wire packet -> version adapter -> WorldEvent -> WorldState`. The persistent network task owns one `WorldState` value and applies events synchronously; the UI and renderer cannot mutate it, and no global lock is introduced. Begin Configuration, Enter World, Respawn, reconfiguration, and Disconnect have explicit replacement/reset semantics. Future chunk/entity stores receive reset scopes without being implemented yet. Generated `GameData` remains an immutable vanilla baseline, while server-authoritative runtime-registry summaries are connection-owned and separate. See `WORLD_STATE.md`.
