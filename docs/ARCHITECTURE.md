...
This document describes current and intended boundaries. Phases 1-12 provide the scaffold, graphics bootstrap, protocol/NBT foundations, networking, Chat Mode/authentication, and generated version/game/packet data. Phase 13 adds a version-independent semantic world/session state whose first wire adapter is Java 26.1.2 / protocol 775. Minecraft gameplay systems and world rendering remain unimplemented.
...
Phases 13-14 follow `exact-version wire packet -> version adapter -> WorldEvent -> WorldState`. The persistent network task owns one `WorldState` and applies events synchronously; the UI and renderer cannot mutate it, and no global lock is introduced. Begin Configuration, Enter World, Respawn, reconfiguration, and Disconnect have explicit reset semantics. Phase 14 attaches a bounded coordinate-keyed semantic chunk store to those hooks. Generated `GameData` remains an immutable vanilla baseline, while server-authoritative runtime registries and typed raw block-state/biome IDs are connection-owned and separate. See `WORLD_STATE.md` and `CHUNKS.md`.
