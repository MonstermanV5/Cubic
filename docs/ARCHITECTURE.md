...
This document describes current and intended boundaries. Phases 1-14 provide the scaffold through bounded semantic chunks. Phase 15 adds a deliberately simple diagnostic terrain renderer; Minecraft gameplay systems, assets, models, and movement remain unimplemented.
...
- `cubic-render`: owns wgpu surface/device submission, Chat Mode's direct `egui-wgpu` paint integration, and Phase 15's depth-tested diagnostic chunk renderer and bounded meshing worker.
...
Phases 13-15 follow `exact-version wire packet -> version adapter -> WorldEvent -> WorldState -> coalesced semantic render deltas`. The persistent network task owns one `WorldState` and applies events synchronously; the UI and renderer cannot mutate it, and no global world lock is introduced. Begin Configuration, Enter World, Respawn, reconfiguration, and Disconnect have explicit reset semantics. Phase 15 shares immutable changed chunks through a bounded latest-value mailbox and performs meshing on a dedicated worker. Generated `GameData` supplies exact air-state classification while server Configuration supplies authoritative dimension geometry. See `WORLD_STATE.md`, `CHUNKS.md`, and `WORLD_RENDERING.md`.
...
The network task retains `MinecraftConnection` after Phase 7 reaches Play. It handles required control/chat packets and Phase 14 chunk lifecycle packets, converts them to protocol-independent events, and discards other complete bounded frames immediately. Chunk state remains network-owned; no entity, inventory, movement, mesh, or terrain-rendering state is built. The UI never owns a socket and rendering never blocks on network I/O.
