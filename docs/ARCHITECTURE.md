...
- `cubic-world`: owns connection-scoped semantic `WorldState`, lifecycle/reset rules, dimensions, player world metadata, authoritative position, spawn/time/difficulty/weather/border state, bounded runtime registries, Phase 19's version-independent DDA raycast/outline-shape semantics, and Phase 20's stable-identity inventory/container/click state. It knows no packet IDs, sockets, UI, renderer, or protocol version.
...
- `cubic-ui`: owns protocol-independent presentation: bounded Chat Mode history/input/connection state and Phase 20's replaceable inventory overlay. It consumes semantic inventory snapshots and emits bounded actions; it owns neither raw packet IDs nor sockets.
...
Phase 19 extends that authority split narrowly for completed block breaks. The network controller predicts only the exact known air result, publishes the copy-on-write semantic chunk immediately, and tracks it by bounded interaction sequence until authoritative update/acknowledgement. Server disagreement always overwrites prediction. Placement is not predicted without Phase 20's authoritative held-item state. Destroy progress crosses the render boundary as a semantic block/state/stage tuple; `cubic-render` resolves the official runtime stage texture and exact selected resource model without placing Minecraft asset bytes in source or protocol constants in generic world code.

Phase 20 follows `generated stable item/menu/component/Creative identity -> protocol-775 codec -> cubic-world inventory state -> cubic-ui inventory overlay`. Runtime numeric IDs and exact packet layouts stay in `cubic-protocol::bootstrap::v775::inventory`. Exact Creative tab metadata, permission variants, ordered stacks, feature requirements, Search membership, and component payloads live in the replaceable `cubic-version` data layer; generic UI code does not infer categories from names or registry order. Networking owns the authoritative connection-scoped state and a bounded action queue; server updates always replace predicted slots/cursor state. The movement controller sees only stable main/offhand item identities, allowing hand-aware use packets, exact-version basic tool speed, and safe property-free placement without putting inventory protocol details into world physics. The renderer resolves the effective default-or-patched item-model component, including generic `minecraft:block_state` selectors, through exact-version item/model graphs, inherited node transformations, special-family parts, and GUI transforms. A shared bounded ModelPart representation composes named parent/child poses, cube-net UVs, face masks, zero-thickness planes, and reflected winding for both special inventory icons and static placed block-entity models; this prevents an ordinary fallback block and special model from being emitted together. A canonical component-consumed render key prevents visually distinct stacks from sharing an atlas cell. Official item definitions, models, textures, screen sprites, localization, and bitmap font bytes are read only from Cubic's verified runtime resource cache. See `INVENTORY.md`.

Phase 17B keeps the solver generic and isolates exact-version physical block behavior in `cubic-world::collision_vanilla`. Phase 20 adds a schema-checked 26.1.2 shape artifact generated by the development-only `tools/ShapeDump.java` oracle: all 29,873 registered states retain separate normalized collision and outline AABB references, while scaffolding, powder snow, and moving pistons are identified as context-dependent families. The generic profiles select those facts by exact runtime ID, identifier, and complete property map; 29,828 state-static collision records and all 29,873 outline records pass a zero-mismatch conversion audit. Visual models, block-entity rendering, collision, selection/outline shapes, and occlusion remain separate. Unknown versions or mismatched datasets remain explicitly approximate rather than silently borrowing current-version IDs.
...
Phase 18B extends the same semantic path rather than adding a renderer-side world. Configuration biome registry data and chunk biome/light palettes remain network-decoded, bounded `cubic-world` state. `cubic-render` receives immutable snapshots, resolves tint/material/fluid/model-offset semantics prepared from the exact-version resource profile, and produces separate solid/cutout/translucent index batches. Selected model elements also provide bounded topology for contained-fluid suppression; generic fluid code does not recognize stair names. Animated PNG frames are bounded resource data and update atlas subregions on the render owner; they never dirty chunk meshes. Light, biome, block, fluid, and neighbour changes reuse coordinate-coalesced revision invalidation. Phase 18C similarly keeps identifiers in the exact-version `BlockEnvironmentProfile`; generic movement queries resolved fluid/contact/surface semantics and never searches block names.
