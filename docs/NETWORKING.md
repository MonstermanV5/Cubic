...
The protocol-775 control plane handles Keep Alive, Ping/Pong, teleport confirmation, cookie absence, chunk-batch rate acknowledgement, one Player Loaded marker after the first batch, and Play-to-Configuration re-entry. Initial Play Client Information is sent. Phase 14 routes bounded chunk loads, unloads, and light updates into the network-owned world store; other complete world/entity frames are still dropped after shared framing limits. Chunk processing emits no UI event and does not change Chat Mode's idle-redraw behavior.
...
World Mode optionally attaches a movement controller to the same persistent Play loop; Chat Mode passes no controller and retains its low-power behavior. A fixed 20 Hz interval samples a coalesced control mailbox, advances semantic simulation, writes only the selected movement/input transition frames, sends the version-profile Client Tick End marker, and publishes the predicted camera pose. The interval skips missed ticks, so stalls do not flood the server. Position/rotation corrections are acknowledged and resolved against current prediction before reseeding packet baselines; delta-velocity flags and local-entity `LpVec3` motion update simulation. Respawn/reconfiguration clears prediction until a new authoritative position arrives. Keep Alive, Ping/Pong, teleport acknowledgement, cookies, chunk batching, chat, and existing control traffic remain in the same loop.

Phase 18 does not add a second network mode. The `world` command's UI stores an atomic protocol-independent `Play`/`Chat` presentation flag beside the existing bounded session channels. It is used only to classify safety warnings such as large corrections while the world is hidden; it neither reconstructs `MinecraftConnection` nor pauses the Play loop. World/chunk/block updates, corrections, keepalives, chat, and the 20 Hz movement clock continue in Chat presentation. Gameplay input is explicitly neutral, so ordinary movement packets converge to stationary state without disconnecting.
...
## Phase 20 inventory transport

World Mode constructs an exact-version inventory profile from generated `GameData` before starting the persistent session. The network task decodes bounded inventory packets before generic Play-frame classification, applies them to one connection-scoped `InventoryState`, and publishes latest-value snapshots to presentation. UI actions travel through a 64-entry FIFO; a full queue is reported instead of dropping an older click. Hotbar selection, close/button actions, and predicted clicks are serialized only by the selected protocol profile. Inventory work never blocks the render thread and Chat Mode does not create inventory controls.

Local click prediction is deterministic and bounded. The outgoing packet carries the server state ID plus changed-slot/cursor fingerprints when Cubic can construct the exact current-version dynamic hash. Regardless of prediction, subsequent full-content or single-slot packets are authoritative and reconcile the local state. Disconnect and world-session reset discard all inventory, quick-craft, and carried state.

Canonical and server-owned patterned banners cross the inventory boundary without becoming opaque or empty: the exact-version profile retains ordered registry-reference or direct pattern holders plus dyes and reconstructs the complete component for Creative Mode Slot writes. In Play, chunk-embedded banner NBT and later Block Entity Data updates use one bounded semantic decoder. A changed block entity copy-on-write updates the authoritative chunk revision and republishes that chunk through the coalescing render mailbox, so a late pattern update replaces a previously plain cached banner mesh.
