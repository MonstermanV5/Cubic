...
`cubic-world` tests use synthetic semantic events only. They cover disconnected/configuring/active lifecycle, arbitrary namespaced dimensions, deterministic known-dimension ordering, entity/game-mode/session flags, finite absolute and relative position updates, stale teleport rejection, spawn/time/difficulty/weather/border updates, same- and cross-dimension respawn invalidation, chunk reset hooks, disconnect/new-session isolation, runtime-registry ownership and caps, clock caps, and forward-compatible semantic game modes. No renderer, launcher file, runtime cache, or server is required.

Phase 14 focused tests add independently authored protocol-775 chunk vectors plus synthetic semantic state. They cover signed coordinates, single/local/direct block and biome palettes, non-crossing packed widths and awkward word boundaries, deterministic decode, malformed indices/lengths/light, bounded compound block-entity NBT summaries, checked section lookup, replacement/unload/store cap, packet-to-semantic adaptation, all reset paths, and receipt during the persistent Chat Mode mock session. Tests use no launcher files, runtime caches, localhost, or external services.

Real Phase 14 localhost acceptance passed against offline-mode vanilla Java 26.1.2. Cubic retained 329 decoded 24-section Overworld chunks, cleared them on transition, retained 329 decoded 16-section Nether chunks, cleared again on return, and reloaded 329 Overworld chunks. Palettes, heightmaps, and light structures decoded without malformed/trailing/bounds errors; Chat Mode remained functional, disconnect was clean, and process memory dropped when each world reset released the previous chunk set. Automated tests remain server-independent.

Phase 15 headless tests cover generated-data air classification, unknown-state rendering policy, authoritative dimension-geometry validation/selection, Configuration Registry Data vectors, exposed-face emission, adjacent/cross-chunk face culling, all-air meshes, mismatched section counts, camera matrix finiteness/aspect changes, neighbor invalidation, and bounded/coalesced reset-safe render deltas. GPU/window appearance remains a manual Windows test; automated suites do not contact a server or require a GPU.

Protocol tests add independent byte vectors for the reviewed 26.1.2 world-state packet subset and malformed/trailing/bounds behavior. Network adapter tests prove packet discriminants and identifiers are rejected at the version boundary and that `cubic-world` receives no numeric packet IDs. The existing persistent in-process Chat Mode server now provides a complete initial Play Login payload and authoritative position synchronization, verifies teleport acknowledgement, and exercises state maintenance without retaining world/chunk traffic.

Real Phase 13 acceptance passed against offline vanilla 26.1.2. Cubic entered authoritative Overworld state, applied initial position/rotation, remained usable in Chat Mode, crossed Overworld -> Nether -> Overworld with `WorldContents` resets and `dimension_changed=true`, invalidated coordinates without losing the independently valid rotation baseline, accepted both post-respawn relative yaw/pitch synchronizations, advanced teleport IDs `1 -> 2 -> 3`, and disconnected cleanly. Automated tests never connect to localhost or a public server.
