...
Phase 9 is `[~]`: Cubic's preferred own-Entra-client-ID path passed real Microsoft OAuth, Xbox User Authentication, and XSTS, then Minecraft Services returned HTTP 403 `Invalid app registration`; this is a confirmed external registration-approval blocker and Cubic does not bypass it. The separate experimental XAL/SISU backend passed real end-to-end authentication through Xbox device authentication, SISU authenticate, OAuth + PKCE, SISU authorize, XSTS, Minecraft `/launcher/login`, entitlement/profile verification, and secure persistence. Windows now replaces the initial manual redirect-copy validation mechanism with a private, top-level-navigation-only WebView2 capture window; its focused real UX retest remains pending. Use of first-party launcher identifiers is not assumed authorized for public distribution. Player certificates, Player Session/secure-chat signing, persistent authenticated Chat Mode, native iOS Keychain/callback integration, and online-mode server acceptance remain incomplete. Phase 10 and later phases have not begun.
