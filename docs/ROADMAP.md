...
- [x] Phase 16 - Minecraft block resources/models
- [x] Phase 17 - basic movement/collision
- [x] Phase 17B - vanilla block collision fidelity
- [x] Phase 18 - seamless Play/Chat mode switching
- [ ] Phase 18B - rendering fidelity
...
Phase 16 passed final real localhost visual acceptance against vanilla Java 26.1.2. World Mode binds exact generated block-state identities to bounded official blockstates/models/textures from Cubic's verified client cache, resolves variants/multipart/inheritance/rotations/UVs, builds a deterministic nearest-sampled atlas with duplicated edge gutters, preserves tint metadata, alpha-tests cutout resources, and emits textured model geometry through bounded near-first mesh workers. Same-coordinate comparisons confirmed block geometry, vertical and direction-specific horizontal texture orientation, atlas edge sampling, vanilla-compatible position-seeded weighted selection, and netherrack rotations. Overworld/Nether rendering and dimension resets, fast visible meshing in the normal DEV build, and local-time log timestamps also passed. Mojang resources remain runtime-only and uncommitted. Fluids, exact biome tint/lighting, animation, translucent sorting, specialized rendering-seed overrides, arbitrary resource-pack layering, movement/entities, deeper Phase 29 optimization, and Phase 30 memory/residency budgets remain explicitly deferred.

Phase 17 passed final manual localhost acceptance against vanilla Java 26.1.2 in the optimized Windows release build. Basic walking acceleration, jumping, sprinting, sprint jumping, retained/chained momentum, collision, local prediction, camera presentation, creative flight, input responsiveness, live block updates, and chunk streaming were accepted as essentially vanilla-like. The corrected model uses the verified 26.1.2 ground/slipperiness, air-control, drag, takeoff-state, and sprint-jump equations; deterministic curves protect acceleration from rest, release deceleration, normal and sprint takeoff, chained/head-hit momentum, and the ordinary-jump distance sanity bound. One isolated small stutter was also perceptible in the official client and disappeared externally after restarting the local server, so it is not a blocker. Comprehensive block collision-shape fidelity and sneak-edge prevention were split into Phase 17B without renumbering Phase 18. Chat Mode remains movement-controller free. Spectator/noclip and special movement mechanics remain deferred.

Phase 17B passed automated validation and optimized Windows localhost acceptance against vanilla Java 26.1.2. The exact-version collision adapter resolves the audited empty, partial, connected, and state-dependent crop/plant families; full cubes; slabs; carpets; snow-layer collision height; farmland/path/soul-sand height; stairs including inner/outer states; doors; trapdoors; fence gates; fences; walls; panes/bars; lily pads; cactus; and representative block-entity-bearing physical states into bounded AABBs. Acceptance covered crops and pumpkin/melon stems, non-full and block-entity-bearing physical states, fence/gate collision above one block, stairs/slabs, ordinary sneak edges, and sneak protection across small downward support transitions. Cocoa, bamboo, young pitcher crops, sea pickles, eggs, flower pots, chorus, and tilted big dripleaves retain their distinct verified shapes; bamboo also uses the exact-version deterministic horizontal offset. Collision depends on the runtime block state, not visual/block-entity rendering support. Connected-family geometry follows the neighbour-derived properties carried by each runtime state. The generic solver preserves shapes outside their owner cell and finds 24/16-high fence/gate geometry through a bounded profile envelope, clips against every collected box, and applies vanilla's 0.05-increment sneak-edge support test. Sneak-edge eligibility follows vanilla's accumulated-fall-distance rule, retaining geometry-driven protection across small downward partial-height transitions while allowing drops beyond the step height and ordinary movement immediately after sneak is released. Unknown states and unknown exact versions remain conservative full cubes and are reported as approximate. Dynamic block-entity collision, fluids, climbing, special block movement modifiers, entity collision, and selection/raycast shapes remain outside Phase 17B.

Phase 18 passed optimized-build manual localhost acceptance. The existing World Mode window switches seamlessly between Play and the existing bounded Chat Mode UI while retaining one network task, connection, world, movement controller, and chat history. Chat presentation keeps protocol/world processing active but performs no 3D frame submission or mesh dispatch; render-side chunk deltas remain coordinate-coalesced and bounded. Returning to Play resumes the existing current-pose near-first scheduler. Gameplay input is cleared and isolated from chat typing. Transition-based damage, low-health, death, dangerous-air, and significant authoritative-displacement alerts use a protocol-independent semantic boundary. Manual acceptance confirmed Play -> Chat -> Play presentation, chat input isolation, persistent session and world processing, return-to-Play convergence, and safety-alert behavior.
